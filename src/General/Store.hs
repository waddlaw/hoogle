{-# LANGUAGE ScopedTypeVariables, RecordWildCards, PatternGuards,
  ViewPatterns, DeriveDataTypeable, GADTs #-}
...
storeWriteFile file act = do
  ...
  withBinaryFile file WriteMode $ \h
   ...
   -> do
    ...
    when (Map.size atoms /= length swAtoms) $ error "Some duplicate names have been written out"
    ...
    let stats =
          ...
          [ (name ++ " :: " ++ atomType, intToDouble atomSize)
          | (name, Atom {..}) <- Map.toList atoms
          ]
    return (stats, res)
...
storeWritePart ::
     forall t a. (Typeable (t a), Typeable a, Stored a)
  => StoreWrite
  -> t a
  -> a
  -> IO ()
...
storeWriteAtom ::
     forall t a. (Typeable (t a), Typeable a)
  => StoreWrite
  -> t a
  -> Bool
  -> CStringLen
...
storeReadAtom ::
     forall a t. (Typeable (t a), Typeable a)
  => StoreRead
  -> t a
  -> (CStringLen -> IO a)
  -> a
storeReadAtom StoreRead {..} (typeOf -> k) unpack =
  unsafePerformIO $ do
    let key = show k
    let val = show $ typeOf (undefined :: a)
    let corrupt msg =
          error $ "The Hoogle file " ++ srFile ++ " is corrupt, " ++ key ++ " " ++ msg ++ "."
...
---------------------------------------------------------------------
-- PAIRS
newtype Fst k v where
  Fst :: k -> Fst k a
  deriving (Typeable)

newtype Snd k v where
  Snd :: k -> Snd k b
  deriving (Typeable)
...
---------------------------------------------------------------------
-- LITERALS
data StoredInt k v where
  StoredInt :: k -> StoredInt k BS.ByteString
  deriving (Typeable)
...
-- JAGGED ARRAYS
data Jagged a =
  Jagged (V.Vector Word32)
         (V.Vector a)
  deriving (Typeable)

data JaggedStore k v where
  JaggedStore :: k -> JaggedStore k (V.Vector Word32, V.Vector a)
  deriving (Typeable)
