        ...
        if null res
          then putStrLn "No results found"
                 ...
                 then do
                   ...
                   let toShow =
                         if numbers && not info
                           then addCounter shown
                           else shown
                   putStr $ unlines toShow
                   when (hidden /= []) $ do
                     whenNormal $
                       putStrLn $
                       "-- plus more results not shown, pass --count=" ++
                       show (count + 10) ++ " to see more"
      else do
        let parseType x =
              case parseQuery x of
                [QueryType t] -> (pretty t, hseToSig t)
                _ -> error $ "Expected a type signature, got: " ++ x
        putStr $
          unlines $ searchTypesDebug store (parseType $ unwords query) (map parseType compare_)

-- | Returns the details printed out when hoogle --info is called
targetInfo :: Target -> String
targetInfo Target {..} =
  unlines $
  [unHTML targetItem] ++ [unwords packageModule | not $ null packageModule] ++ [unHTML targetDocs]
  where
...
targetResultDisplay link Target {..} =
  unHTML $
  unwords $ fmap fst (maybeToList targetModule) ++ [targetItem] ++ ["-- " ++ targetURL | link]
...
search store qs =
  runIdentity $ do
    ...
    is <-
      case (filter isQueryName qs, filter isQueryType qs) of
        ([], []) -> return list
...
action_search_test sample database =
  testing "Action.Search.search" $
  withSearch database $ \store -> do
    let noResults a = do
    ...
    if sample
      then do
        ...
        let asserts b x =
              if b
                then putChar '.'
                else error $ "Assertion failed, got False for " ++ x
